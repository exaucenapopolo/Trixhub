    ...
    <Layout>
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {user?.isFreeAccount && !user?.isActivated && (() => {
          ...
          return (
            <div className="bg-gradient-to-br from-amber-500/10 to-primary/5 border border-amber-500/25 rounded-2xl p-5 space-y-4">
              {/* En-tête + progression */}
              ...
              <div className="border-t border-amber-500/15 pt-3 space-y-2">
                <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wider">Active ton compte maintenant</p>
                <div className="flex flex-col gap-2">
                  <Link
                    href="/free-account"
                    className="w-full inline-flex items-center justify-center gap-2 px-3 py-3 rounded-xl bg-green-600 text-white text-xs font-bold hover:bg-green-700 transition-colors shadow-sm"
                  >
                    <Zap className="w-3.5 h-3.5" /> Payer le reste — {Math.ceil(Math.max(0, 3600 - parseFloat(user.activationCredit ?? "0"))).toLocaleString("fr-FR")} FCFA seulement
                  </Link>
                  <div className="flex gap-2">
                    <Link
                      href="/activate"
                      className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl bg-muted text-muted-foreground text-xs font-medium hover:bg-muted/80 transition-colors border border-border"
                    >
                      <Zap className="w-3 h-3" /> Payer 3 600 FCFA
                    </Link>
                    <Link
                      href="/depot"
                      className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 text-xs font-bold hover:bg-emerald-500/25 transition-colors border border-emerald-500/25"
                    >
                      <PiggyBank className="w-3.5 h-3.5" /> Déposer
                    </Link>
                  </div>
                </div>
                <p className="text-[10px] text-muted-foreground leading-snug">
                  "Payer le reste" utilise ton crédit accumulé — tu ne paies que le manquant. Le parrain reçoit sa commission normalement.
                </p>
              </div>
            </div>
          );
